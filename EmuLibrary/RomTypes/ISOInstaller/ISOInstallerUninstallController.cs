...
namespace EmuLibrary.RomTypes.ISOInstaller
{
    class ISOInstallerUninstallController : UninstallController
    {
        ...
        internal ISOInstallerUninstallController(Game game, IEmuLibrary emuLibrary) : base(game)
        {
            Name = "Uninstall";
                ...
                try
                {
                    ...
                    if (File.Exists(uninstallerPath))
                    {
                        ...
                        using (var process = System.Diagnostics.Process.Start(startInfo))
                        {
                            ...
                            _emuLibrary.Logger.Info($"Uninstaller completed for {Game.Name}");
                            _emuLibrary.Logger.Info($"Uninstaller completed for {Game.Name} with exit code {process.ExitCode}");
                        }
                    }
                    else
                    {
                        // If no uninstaller found, just delete the directory
